...
function basicAuth(oo: OpenObserveConfig): string | null {
  if (oo.token) return `Basic ${oo.token}`;
...
export function createLogger(config: LoggerConfig): KitLogger {
  ...
  function ship(record: Record<string, unknown>): void {
    if (!enabled) return;
