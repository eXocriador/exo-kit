...
/** An address range, pre-decoded so matching is two BigInt ops. */
interface Cidr {
  base: bigint;
  mask: bigint;
}
...
function decodeIPv4(text: string): bigint | null {
 ...
 */
function decodeIPv6(text: string): bigint | null {
  let body = text;
  ...
  if (runs.length > 2) return null; // '::' may appear at most once
...
function cidr(notation: string): Cidr {
  ...
  if (decoded === null) throw new Error(`ssrf-guard: malformed CIDR ${notation}`);
 ...
 */
const RESERVED_V4: readonly Cidr[] = [
 ...
 */
const RESERVED_V6: readonly Cidr[] = [
  '::/128', //           unspecified
  '::1/128', //          loopback
...
function matches(value: bigint, blocks: readonly Cidr[]): boolean {
 ...
 */
const RESERVED_NAMES: readonly RegExp[] = [
  /^localhost$/,
  /\.localhost$/,
...
export async function validateHost(host: string): Promise<ValidationResult> {
  ...
  if (!name) return { ok: false, reason: 'empty host' };
  ...
  if (family === 4) {
    ...
    if (isReservedIPv4(canonical)) return { ok: false, reason: 'IPv4 in reserved range' };
  ...
  if (!HOSTNAME.test(name)) return { ok: false, reason: 'invalid hostname syntax' };
  ...
  if (answers.length === 0) return { ok: false, reason: 'hostname has no A/AAAA records' };
...
export async function safeFetch(
  ...
  let url = inputUrl;
  let requestInit: RequestInit = passInit;
  ...
  for (let hop = 0; hop <= maxRedirects; hop++) {
    ...
    if (!location) return res; // a 3xx with nowhere to go is the caller's to interpret

    const next = new URL(location, url);
    if (next.origin !== parsed.origin) requestInit = stripCredentials(requestInit);
    url = next.toString();
  }
