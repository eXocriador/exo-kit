 ...
 * API.
 */
export function createConnectorHandler(opts: ConnectorHandlerOptions) {
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    if (!secrets.length) return json({ error: 'connector is not configured' }, 503);
    ...
    const path = new URL(req.url).pathname.replace(
      /.*?(\/facts|\/identity\/resolve|\/tools\/[^/]+)$/,
      '$1',
    );

    if (path === '/facts') return json(await opts.facts());

    if (path === '/identity/resolve') {
      if (!opts.resolveIdentity) return json(null);
    ...
    if (tool) {
      ...
      if (!fn) return json({ error: `tool "${tool}" not implemented` }, 404);
      const { userId } = body ? (JSON.parse(body) as { userId?: string }) : {};
